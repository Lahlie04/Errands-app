...
const getRequestByClientId = "SELECT s.name AS service, r.comment, r.status, r.req_date FROM request r JOIN service s ON r.service_id = s.service_id WHERE r.client_id = $1"

const getRequest = "SELECT  s.name AS service, concat(u.name ,' ', u.surname) AS client_name, r.comment, concat(a.street_address,', ', a.suburb,', ',  a.city,', ',  a.postal_code) AS address, FROM request r, service s, address a WHERE r.client_id=u.id AND  r.service_id = s.id   AND a.request_id=r.id AND r.address_id=a.id  AND r.status = 'pending' ";

const getRequestByRunnerId = "SELECT s.name AS service , concat(u.name ,' ', u.surname) AS client_name, concat(a.street_address,', ', a.suburb,', ',  a.city,', ',  a.postal_code) AS address, status, r.id,  FROM request r,users u, service s, address a WHERE r.client_id=u.id AND r.service_id = s.id AND a.request_id=r.id AND runner_id =$1 ORDER BY req_date DESC";
;
