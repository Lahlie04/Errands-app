...
const updateStatus = "UPDATE request SET status =$1 WHERE id=$2 RETURNING *";
...
const getRequestByClientId = "SELECT r.id, s.name AS service , r.comment, r.status, r.req_date FROM request r JOIN service s ON r.service_id = s.id WHERE r.client_id=$1"
...
const getRequestByRunnerId = "SELECT r.id, s.name AS service , concat(u.name ,' ', u.surname) AS client_name, concat(a.street_address,', ', a.suburb,', ',  a.city,', ',  a.postal_code) AS address, status FROM request r,users u, service s, address a WHERE r.client_id=u.id AND r.service_id = s.id AND r.address_id=a.id AND runner_id = $1 ORDER BY req_date DESC";

// const getRequestByRunnerId = "SELECT s.name AS service , r.comment, concat(u.name ,' ', u.surname) AS client_name, r.status, r.req_date, address_id FROM address a, request r, users u JOIN service s ON r.service_id = s.id WHERE r.address_id = a.id AND r.runner_id = $1";
